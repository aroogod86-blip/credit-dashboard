...
<div class="tabs">
  ...
  <div class="tab" data-tab="t8">💰 발행자 재무</div>
...
<div class="tp" id="t8">
  ...
  <div id="t8-chart-panel" style="display:none;padding:0 32px 24px">
    <div style="background:var(--sf);border:1px solid var(--bd);border-radius:10px;padding:16px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        <div id="t8-chart-title" style="font-weight:600;font-size:14px"></div>
        <div style="display:flex;gap:6px;align-items:center">
          ...
          <span style="cursor:pointer;font-size:13px;color:var(--tx3)" onclick="document.getElementById('t8-chart-panel').style.display='none'">✕ 닫기</span>
        </div>
      </div>
      <canvas id="t8-chart-canvas" height="90"></canvas>
    </div>
  </div>
</div>
</div>
...
let t8sc={{corp:'ebitda_to_interest', fin:'tier1_ratio'}};
let t8sd={{corp:-1, fin:-1}};
...
function getT8Group(isFinancial){{
  ...
  const groupKey = isFinancial ? 'fin' : 'corp';
  let arr = Object.keys(src).map(function(ticker){{
    const d = src[ticker];
    return Object.assign({{ticker:ticker}}, d);
  }});
  arr = arr.filter(function(d){{ return (d.is_financial===true) === isFinancial; }});
...
function renderT8Table(groupKey, thId, tbId, countId, isFinancial, metricsList){{
  const rows = getT8Group(isFinancial);
...
function R8(){{
  ...
  renderT8Table('corp', 't8-corp-th', 't8-corp-tb', 't8-corp-count', false, T8_METRICS_CORP);
  renderT8Table('fin',  't8-fin-th',  't8-fin-tb',  't8-fin-count',  true,  T8_METRICS_FIN);
}}
...
function openT8Chart(ticker, groupKey){{
  ...
  document.getElementById('t8-chart-panel').style.display='block';
  renderT8Chart();
}}
